                ...
                {this.props.items.map(movie => {
                    ...
                    <div key={movie.id} style={{background: "black"}} className="swiper-slide">
                        <Link id="link" style={{color: "rgba(255, 255, 255, .7)"}} to={`/details/${this.props.type.toLowerCase()}/${movie.id}`}>
                            <div className="item">
                                <img className="swiper-slide__image wow fadeInDown" data-wow-delay=".5s" data-wow-duration="1s" src={`${this.props.MDBConfig.images.secure_base_url}w154${movie.poster_path}`} alt={movie.name}/>
                                <p className="swiper-slide__title wow fadeIn" data-wow-delay=".5s" data-wow-duration="1s" >{movie.name}</p>
